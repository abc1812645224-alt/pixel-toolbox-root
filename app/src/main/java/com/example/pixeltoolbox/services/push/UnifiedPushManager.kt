...
object UnifiedPushManager {
    ...
    suspend fun enableTencentSpoof(context: Context): Result<String> = withContext(Dispatchers.IO) {
        val cmds = listOf(
            ...
            "resetprop ro.product.manufacturer Xiaomi 2>/dev/null",
            "resetprop ro.product.brand Xiaomi 2>/dev/null",
            "setprop ro.miui.ui.version.name V140 2>/dev/null",
            "cmd appops set $XMSF_PACKAGE RUN_IN_BACKGROUND allow 2>/dev/null",
            "cmd appops set $XMSF_PACKAGE WAKE_LOCK allow 2>/dev/null",
            "cmd appops set $XMSF_PACKAGE AUTO_START allow 2>/dev/null",
            "cmd appops set $WECHAT_PACKAGE RUN_IN_BACKGROUND allow 2>/dev/null",
            "cmd appops set $WECHAT_PACKAGE WAKE_LOCK allow 2>/dev/null",
            "cmd appops set $WECHAT_PACKAGE AUTO_START allow 2>/dev/null",
            "dumpsys deviceidle whitelist +$XMSF_PACKAGE 2>/dev/null",
            "dumpsys deviceidle whitelist +$WECHAT_PACKAGE 2>/dev/null",
            "am broadcast -a com.xiaomi.mipush.RECEIVE_MESSAGE -n com.tencent.mm/com.tencent.mm.plugin.mipush.MiPushBroadcastReceiver -f 32 2>/dev/null",
